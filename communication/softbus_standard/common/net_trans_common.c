...
static void OnDefNodeOnline(NodeBasicInfo* info)
{
  ...
  if (strncpy_s(g_networkId, NETWORK_ID_BUF_LEN, info->networkId,
...
int SendCtrlMsgToRemote(CtrlCodeType code)
{
  ...
  switch (code) {
    ...
    case CTRL_CODE_CHANGE_WIFI_SIXTY_SEC:
      ret = SendMessage(g_currentSessionId4Ctl, CTRL_MSG_CHANGE_WIFI_SIXTY_SEC,
    ...
    case CTRL_CODE_OPEN_SESSION_NOT_EXIST:
      ret = SendMessage(g_currentSessionId4Ctl, CTRL_MSG_OPEN_SESSION_NOT_EXIST,
                              ...
                              char* netId) 
{
  ...
  g_openSessionTimeStart = GetCurrentTimeOfMs();
  sessionId =
      OpenSession(sessionName, sessionName, netId, groupId, g_sessionAttr4Perf);
  if (sessionId < SESSION_ID_MIN) {
    LOG("call OpenSession[perf] fail, ret sid:%d", sessionId);
    return SOFTBUS_ERR;
  }
...
void* OpenSessionTask4Ctl(void* param) 
{
  ...
  char groupIdMore[][GROUP_ID_LEN] = {"m1", "m2", "m3"};
  for (int i = 0; i < moreCount; i++) {
    int sessionId = OpenSession(SESSION_NAME_CTL, SESSION_NAME_CTL, g_networkId,
...
int CheckRemoteDeviceIsNull(int isSetNetId) 
{
  ...
  if (nodeInfo != NULL && nodeNum > 0) {
    ...
    if (isSetNetId == BOOL_TRUE) {
      if (strncpy_s(g_networkId, NETWORK_ID_BUF_LEN, nodeInfo->networkId,
                    NETWORK_ID_BUF_LEN) != 0) {
        return SOFTBUS_ERR;
      }
    }
  ...
  if (nodeInfo != NULL && nodeNum > 0) {
    for (int i = 0; i < nodeNum; i++) {
      ...
      *(netId + i) = (char*)malloc(NETWORK_ID_BUF_LEN * sizeof(char));
      if (strncpy_s(*(netId + i), NETWORK_ID_BUF_LEN, nodeInfo->networkId,
                    NETWORK_ID_BUF_LEN) != 0) {
...
int SetRemoteDeviceNetIdToGarray(void) 
{
  ...
  if (nodeInfo != NULL && nodeNum > 0) {
    for (int i = 0; i < nodeNum; i++) {
      if (strncpy_s(g_networkIdArray[i], NETWORK_ID_BUF_LEN,
                    nodeInfo->networkId, NETWORK_ID_BUF_LEN) != 0) {
        FreeNodeInfo(nodeInfoTmp);
        return SOFTBUS_ERR;
      }
...
int SetCurrentNetworkId(int index) 
{
  ...
  int ret = strncpy_s(g_networkId, NETWORK_ID_BUF_LEN, g_networkIdArray[index],
                      NETWORK_ID_BUF_LEN);
  if (ret != SOFTBUS_OK) {
