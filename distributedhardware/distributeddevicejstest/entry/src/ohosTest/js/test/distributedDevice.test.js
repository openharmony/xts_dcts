/*
 * Copyright (C) 2022-2024 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import { describe, beforeAll, beforeEach, afterEach, afterAll, it, expect, TestType, Size, Level } from '@ohos/hypium'
import deviceManager from '@ohos.distributedDeviceManager';
import featureAbility from "@ohos.ability.featureAbility";
import { UiDriver, BY } from '@ohos.UiTest';
import TestService from "./TestService.test";

let testservice = new TestService();
let dmInstance = null;
let dmNetworkId = null;
let dmDeviceInfo = null;
let deviceId = null;
let localDeviceId = null;
const TEST_BUNDLE_NAME = 'com.ohos.distributedscreenjstest';
let TAG = 'distributedDeviceManager';

export default function distributedDeviceManager() {

    describe('distributedDeviceManager', function () {

        function sleep(ms) {
            return new Promise(resolve => setTimeout(resolve, ms));
        }

        async function getPermission() {
            console.info(`getPermission is start`);
            let permissions = ['ohos.permission.DISTRIBUTED_DATASYNC'];
            let context = featureAbility.getContext();
            context.requestPermissionsFromUser(permissions, 666, (data) => {
                console.info("request success" + JSON.stringify(data));
            })
        }

        async function driveFn() {
            try {
                let driver = await UiDriver.create();
                console.info(`come in driveFn`);
                console.info(`driver is ${JSON.stringify(driver)}`);
                await sleep(1000);
                let button = await driver.findComponent(BY.text('允许'));
                console.info(`button is ${JSON.stringify(button)}`);
                await sleep(1000);
                await button.click();
                await sleep(1000);
            } catch (err) {
                console.info('err is ' + err);
                return;
            }
        }

        async function getDeviceId() {
            console.log('getDeviceId is begin')
            try {
                dmInstance = deviceManager.createDeviceManager('com.ohos.distributedscreenjstest');
                console.log('get deviceManager is success')
            } catch (error) {
                console.log('get deviceManager is failed' + JSON.stringify(error))
            }
            localDeviceId = dmInstance.getLocalDeviceNetworkId();
            console.info("local device id is: " + localDeviceId);
            let deviceList = dmInstance.getAvailableDeviceListSync();
            deviceId = deviceList[0].networkId;
            console.info("deviceid is: " + deviceId);
            console.info("deviceList is: " + JSON.stringify(deviceList));
        }

        //检查当前应用是否有可信的设备
        async function checkAvailableDevice() {
            console.info(TAG + "checkAvailableDevice begin");
            let dmInstance = deviceManager.createDeviceManager('com.ohos.distributedscreenjstest');
            let deviceInfoList = dmInstance.getAvailableDeviceListSync();
            console.info(TAG + "checkAvailableDevice get deviceInfoList " + JSON.stringify(deviceInfoList));
            if (deviceInfoList.length != 0) {
                console.info(TAG + "false deviceInfoList.length is" + JSON.stringify(deviceInfoList));
                return false;
            } else {
                console.info(TAG + "true deviceInfoList.length  is" + JSON.stringify(deviceInfoList));
                return true;
            }
        }

        async function checkResult() {
            console.info(TAG + "checkResult begin");
            try {
                let checkResult = await checkAvailableDevice();
                console.info(TAG + "checkResult is" + checkResult);
                if (!checkResult) {
                    testservice.unbindStub();
                }
                await sleep(500);
                let checkResult1 = await checkAvailableDevice();
                //如果有可信的设备 就不需要再通过PIN码bind
                if (checkResult1) {
                    testservice.startDiscovering();
                    await sleep(3000);
                    testservice.bindStub();
                    await sleep(20000);
                    testservice.stopDiscovering();
                    await sleep(3000);
                }
            } catch (err) {
                console.info('err is ' + err);
                return;
            }
        }

        beforeAll(async function (done) {
            console.info("beforeAll start");
            await getPermission();
            await sleep(1000);
            await driveFn();
            await sleep(1000);
            await checkResult()
            await sleep(3000);
            await getDeviceId()
            await sleep(1000);
            console.info("beforeAll done");
            done();
        })

        beforeEach(async function (done) {
            console.info(TAG + 'beforeEach start');
            let dmInstance = deviceManager.createDeviceManager(TEST_BUNDLE_NAME);
            expect(dmInstance !== null && dmInstance !== undefined).assertTrue();
            console.info(TAG + 'beforeEach get dmInstance is successfully');
            done();
        })

        afterEach(async function (done) {
            console.info('afterEach called');
            setTimeout(done, 500);
        })

        afterAll(async function (done) {
            console.info('afterAll called');
            console.info('afterAll start');
            // 删除当前应用的可信设备
            let checkResult = await checkAvailableDevice();
            console.info('afterAll checkResult is ' + checkResult);
            if (!checkResult) {
                console.info('checkResult is ' + checkResult);
                testservice.unbindStub();
            }
            await sleep(2000);
            await driveClick();
            await sleep(1000);
            console.info('afterAll start');
            done();
        })


        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0100 start------------------------");
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.error("createDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                    expect().assertFail();
                    done();
                }
                console.info("createDeviceManager success");
                dmInstance.on('replyResult', (data) => console.log("replyResult on:" + JSON.stringify(data)));
                dmInstance.on('discoverFail', (data) => console.log("discoverFail on:" + JSON.stringify(data)));
                dmInstance.on('serviceDie', (data) => console.log("serviceDie on:" + JSON.stringify(data)));
                dmInstance.on('deviceNameChange', (data) => console.log("deviceNameChange on:" + JSON.stringify(data)));
                dmInstance.on('deviceStateChange', (data) => console.log("deviceStateChange on:" + JSON.stringify(data)));
                expect(dmInstance !== null).assertTrue();
                done();
            } catch (err) {
                console.error("createDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0200
         * @tc.name    Start to discover nearby devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0200 start------------------------");
            var discoverParam = {
                "discoverTargetType": 1
            };
            let data = null;
            let flag = 0;
            try {
                dmInstance.on('discoverSuccess', (data) => {
                    console.info("discoverSuccess:" + JSON.stringify(data));
                });

                dmInstance.startDiscovering(discoverParam);
                console.log("startDiscovering success");
                expect(flag == 0).assertTrue();
                done();
            } catch (err) {
                console.error("startDiscovering errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0300
         * @tc.name    Get the network id of the local device
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0300 start------------------------");
            try {
                dmNetworkId = dmInstance.getLocalDeviceNetworkId();
                if (dmNetworkId === null) {
                    console.error("getLocalDeviceNetworkId fail");
                }
                console.log("getLocalDeviceNetworkId dmNetworkId:" + dmNetworkId);
                expect(dmNetworkId !== null).assertTrue();
                done();
            } catch (err) {
                console.error("getLocalDeviceNetworkId errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0400
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0400 start------------------------");
            try {
                dmDeviceInfo = dmInstance.getAvailableDeviceListSync();
                if (dmDeviceInfo === null) {
                    console.log('getAvailableDeviceListSync info fail');
                }
                console.log('getAvailableDeviceListSync info:' + dmDeviceInfo);
                console.log('getAvailableDeviceListSync info:' + JSON.stringify(dmDeviceInfo[0]["deviceId"]));
                expect(dmDeviceInfo !== null).assertTrue();
                done();
            } catch (err) {
                console.error("getAvailableDeviceListSync errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0500
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0500 start------------------------");
            try {
                dmInstance.getAvailableDeviceList((err, data) => {
                    if (err) {
                        console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                    expect(data !== null).assertTrue();
                    done();
                });
            } catch (err) {
                console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0600
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0600 start------------------------");
            dmInstance.getAvailableDeviceList().then((data) => {
                console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                expect(data !== null).assertTrue();
                done();
            }).catch((err) => {
                console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            })
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0700
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0700 start------------------------");
            try {
                dmDeviceInfo = dmInstance.getAvailableDeviceListSync();
                if (dmDeviceInfo === null) {
                    console.log('getAvailableDeviceListSync info fail');
                }
                console.log('getAvailableDeviceListSync info:' + dmDeviceInfo);
                console.log('getAvailableDeviceListSync info:' + JSON.stringify(dmDeviceInfo[0]["deviceId"]));
                expect(dmDeviceInfo[0] != null).assertTrue();
                done();
            } catch (err) {
                console.error("getAvailableDeviceListSync errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2900
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2900 start------------------------");
            try {
                dmInstance.getAvailableDeviceList((err, data) => {
                    if (err) {
                        console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                    expect(data[0] != null).assertTrue();
                    done();
                });
            } catch (err) {
                console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6700
         * @tc.name    Get a list of available devices. This interface query all authorized and connectable devices.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6700 start------------------------");
            dmInstance.getAvailableDeviceList().then((data) => {
                console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                expect(data[0] !== null).assertTrue();
                done();
            }).catch((err) => {
                console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            })
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0800
         * @tc.name    Get the device name of the local device.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0800 start------------------------");
            try {
                let lDeviceName = dmInstance.getLocalDeviceName();
                if (lDeviceName === null) {
                    console.log("getLocalDeviceName fail");
                }
                console.log("Get Local Device Name:" + lDeviceName);
                expect(lDeviceName !== null).assertTrue();
                done();
            } catch (err) {
                console.info("getLocalDeviceName errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_0900
         * @tc.name    Get the device type of the local device
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_0900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0900 start------------------------");
            try {
                let lDeviceType = dmInstance.getLocalDeviceType();
                if (lDeviceType === null) {
                    console.log("getLocalDeviceType fail");
                }
                console.log("get Local Device Type:" + lDeviceType);
                expect(lDeviceType !== null).assertTrue();
                done();
            } catch (err) {
                console.info("getLocalDeviceType errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_0900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1000
         * @tc.name    Get the device id of the local device
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1000 start------------------------");
            try {
                let lDeviceId = dmInstance.getLocalDeviceId();
                if (lDeviceId === null) {
                    console.log("getLocalDeviceId fail");
                }
                console.log("get Local Device Id:" + lDeviceId);
                expect(lDeviceId !== null).assertTrue();
                done();
            } catch (err) {
                console.info("getLocalDeviceId errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1100
         * @tc.name    Get the device name by network id.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1100 start------------------------");
            try {
                let deviceName = dmInstance.getDeviceName(dmNetworkId);
                if (deviceName === null) {
                    console.log("getDeviceName fail");
                }
                console.log("get Device Name:" + deviceName);
                expect(deviceName !== null).assertTrue();
                done();
            } catch (err) {
                console.info("getDeviceName errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1200
         * @tc.name    Get the device type by network id.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1200 start------------------------");
            try {
                let deviceType = dmInstance.getDeviceType(dmNetworkId);
                if (deviceType === null) {
                    console.log("getDeviceType fail");
                }
                console.log("get Device Type:" + deviceType);
                expect(deviceType !== null).assertTrue();
                done();
            } catch (err) {
                console.info("getDeviceType errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1200 end------------------------");
        })

        /*
          * @tc.number  SUB_DH_DeviceManager_Dcts_1400
          * @tc.name    Stop discovering nearby devices.
          * @tc.desc    Function test
          * @tc.size    MediumTest
          * @tc.type:   Function
          * @tc.level   Level1
          */
        it("SUB_DH_DeviceManager_Dcts_1400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1400 start------------------------");
            let flag = 0;
            try {
                dmInstance.stopDiscovering();
                console.log("stopDiscovering success");
                expect(flag == 0).assertTrue();
                done();
            } catch (err) {
                console.info("stopDiscovering errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1500
         * @tc.name    Releases the {@code DeviceManager} instance that is no longer used.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1500 start------------------------");
            var mFilterOption = {
                targetPkgName: "com.ohos.distributedscreenjstest",
                sortType: 0,
                filter: JSON.stringify({
                    key: 'test',
                    value: 0
                })
            }
            let flag = 0;
            try {
                dmInstance.off('replyResult');
                dmInstance.off('discoverSuccess');
                dmInstance.off('discoverFail');
                dmInstance.off('serviceDie');
                dmInstance.off('deviceStateChange');
                dmInstance.off('deviceNameChange');
                deviceManager.releaseDeviceManager(dmInstance);
                console.log("releaseDeviceManager success");
                expect(flag == 0).assertTrue();
                done();
            } catch (err) {
                console.info("releaseDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1600
         * @tc.name    Releases the {@code DeviceManager} instance that is no longer used.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1600 start------------------------");
            var mFilterOption = {
                targetPkgName: "com.ohos.distributedscreenjstest",
                sortType: 0,
                filter: JSON.stringify({
                    key: 'test',
                    value: 0
                })
            }
            try {
                deviceManager.releaseDeviceManager(dmInstance);
                console.log("releaseDeviceManager success");
                dmInstance.off('replyResult');
                dmInstance.off('discoverSuccess');
                dmInstance.off('discoverFail');
                dmInstance.off('serviceDie');
                dmInstance.off('deviceStateChange');
                dmInstance.off('deviceNameChange');
                expect().assertFail();
                done();
            } catch (err) {
                console.info("releaseDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1700
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1700 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1700 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_1700 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_1700 releaseDeviceManager success");
                    deviceInfoList = dmInstance.getAvailableDeviceListSync();
                    if (!deviceInfoList) {
                        console.log("getAvailableDeviceListSync three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceInfoList != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getAvailableDeviceListSync SUB_DH_DeviceManager_Dcts_1700 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1700 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1800 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1800 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_1800 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_1800 releaseDeviceManager success");
                    dmInstance.getAvailableDeviceList((err, data) => {
                        if (err) {
                            console.log("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                        }
                        console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                        expect(data !== null).assertTrue();
                        done();
                    });
                } catch (err) {
                    console.log("getAvailableDeviceList SUB_DH_DeviceManager_Dcts_1800 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1800 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1900
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1900 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1900 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_1900 success");

                deviceManager.releaseDeviceManager(dmInstance);
                console.info("SUB_DH_DeviceManager_Dcts_1900 releaseDeviceManager success");
                dmInstance.getAvailableDeviceList().then((data) => {
                    console.log('getAvailableDeviceList SUB_DH_DeviceManager_Dcts_1900 info: ' + JSON.stringify(data));
                    expect(data !== null).assertTrue();
                    done();
                }).catch((err) => {
                    console.log("getAvailableDeviceList SUB_DH_DeviceManager_Dcts_1900 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                })
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1900 four errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2000
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2000 start------------------------");
            let deviceNetworkId = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2000 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2000 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2000 releaseDeviceManager success");

                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2000 releaseDeviceManager success");
                    console.log('local device networkId succes');
                    deviceNetworkId = dmInstance.getLocalDeviceNetworkId();
                    if (!deviceNetworkId) {
                        console.log("getLocalDeviceNetworkId three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    console.log('local device networkId: ' + JSON.stringify(deviceNetworkId));
                    expect(deviceNetworkId != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getLocalDeviceNetworkId SUB_DH_DeviceManager_Dcts_2000 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2000 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2100 start------------------------");
            let deviceName = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2100 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2100 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2100 releaseDeviceManager success");

                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2100 releaseDeviceManager success");
                    deviceName = dmInstance.getLocalDeviceName();
                    if (!deviceName) {
                        console.log("getLocalDeviceName three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceName != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getLocalDeviceName SUB_DH_DeviceManager_Dcts_2100 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2100 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2200
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2200 start------------------------");
            let deviceType = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2200 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2200 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2200 releaseDeviceManager success");

                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2200 releaseDeviceManager success");
                    deviceType = dmInstance.getLocalDeviceType();
                    if (!deviceType) {
                        console.log("getLocalDeviceType three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceType != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getLocalDeviceType SUB_DH_DeviceManager_Dcts_2200 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2200 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2300 start------------------------");
            let deviceId = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2300 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2300 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2300 releaseDeviceManager success");

                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2300 releaseDeviceManager success");
                    deviceId = dmInstance.getLocalDeviceId();
                    if (!deviceId) {
                        console.log("getLocalDeviceId three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceId != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getLocalDeviceId SUB_DH_DeviceManager_Dcts_2300 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2300 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2400
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2400 start------------------------");
            let deviceName = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2400 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2400 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2400 releaseDeviceManager success");
                    deviceName = dmInstance.getDeviceName(dmNetworkId);
                    if (!deviceName) {
                        console.log("getDeviceName three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceName != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getDeviceName SUB_DH_DeviceManager_Dcts_2400 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2400 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2500
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2500 start------------------------");
            let deviceType = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2500 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2500 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2500 releaseDeviceManager success");
                    deviceType = dmInstance.getDeviceType(dmNetworkId);
                    if (!deviceType) {
                        console.log("getDeviceType three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceType != null).assertTrue();
                    done();
                } catch (err) {
                    console.log("getDeviceType SUB_DH_DeviceManager_Dcts_2500 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2500 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2600
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2600 start------------------------");
            let deviceType = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2600 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2600 success");
                var discoverParam = {
                    "discoverTargetType": 1
                };
                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2600 releaseDeviceManager success");
                    dmInstance.startDiscovering(discoverParam);
                    expect().assertFail();
                    done();
                } catch (err) {
                    console.log("startDiscovering SUB_DH_DeviceManager_Dcts_2600 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2600 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2700
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2700 start------------------------");
            let deviceType = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2700 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2700 success");

                let bindParam = {
                    "bindType": 1,
                    "targetPkgName": "com.ohos.distributedDeviceManager",
                    "appName": "SubDctsdisDeviceJsTest",
                    "appOperation": "想要连接本机。",
                    "customDescription": "device manager"
                };

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2700 releaseDeviceManager success");
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2700 releaseDeviceManager success");
                    dmInstance.bindTarget(deviceId, bindParam, (err, data) => {
                        if (err) {
                            console.info("bindTarget fail");
                        }
                        console.info("bindTarget result:" + JSON.stringify(data));
                        expect(data != null).assertTrue();
                        done();
                    });
                } catch (err) {
                    console.log("bindTarget SUB_DH_DeviceManager_Dcts_2700 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2700 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_2800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_2800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2800 start------------------------");
            let deviceType = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2800 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_2800 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2800 releaseDeviceManager success");
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_2800 releaseDeviceManager success");
                    dmInstance.unbindTarget(deviceId);
                    expect().assertFail();
                    done();
                } catch (err) {
                    console.log("unbindTarget SUB_DH_DeviceManager_Dcts_2800 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_2800 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_2800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3000
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3000 start------------------------");

            try {
                dmInstance.on('replyResult', (data) => {
                    console.log("replyResult executed, dialog closed" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("replyResult SUB_DH_DeviceManager_Dcts_3000 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3100 start------------------------");
            try {
                dmInstance.off('replyResult', (data) => {
                    console.log("replyResult executed, dialog closed" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("replyResult SUB_DH_DeviceManager_Dcts_3100 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3200
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3200 start------------------------");
            try {
                dmInstance.on('deviceStateChange', (data) => {
                    console.log("deviceStateChange on:" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceStateChange SUB_DH_DeviceManager_Dcts_3200 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3300 start------------------------");
            try {
                dmInstance.off('deviceStateChange', (data) => {
                    console.info('deviceStateChange' + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceStateChange SUB_DH_DeviceManager_Dcts_3300 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3400
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3400 start------------------------");
            try {
                dmInstance.on('discoverSuccess', (data) => {
                    console.info("discoverSuccess:" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("discoverSuccess SUB_DH_DeviceManager_Dcts_3400 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3500
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3500 start------------------------");
            try {
                dmInstance.off('discoverSuccess', (data) => {
                    console.info('discoverSuccess' + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("discoverSuccess SUB_DH_DeviceManager_Dcts_3500 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3600
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3600 start------------------------");
            try {
                dmInstance.on('deviceNameChange', (data) => {
                    console.info("deviceNameChange on:" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceNameChange SUB_DH_DeviceManager_Dcts_3600 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3700
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3700 start------------------------");
            try {
                dmInstance.off('deviceNameChange', (data) => {
                    console.info('deviceNameChange' + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceNameChange SUB_DH_DeviceManager_Dcts_3700 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3800 start------------------------");
            try {
                dmInstance.on('discoverFailure', (data) => {
                    console.info("discoverFailure on:" + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("discoverFailure SUB_DH_DeviceManager_Dcts_3800 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_3900
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_3900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3900 start------------------------");
            try {
                dmInstance.off('discoverFailure', (data) => {
                    console.info('discoverFailure' + JSON.stringify(data));
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("discoverFailure SUB_DH_DeviceManager_Dcts_3900 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_3900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4000
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4000 start------------------------");
            try {
                dmInstance.on("serviceDie", (data) => {
                    console.info("serviceDie on");
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("serviceDie on SUB_DH_DeviceManager_Dcts_4000 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4100 start------------------------");
            try {
                dmInstance.off("serviceDie", (data) => {
                    console.info("serviceDie off");
                    expect().assertFail();
                    done();
                });
            } catch (err) {
                console.log("serviceDie off SUB_DH_DeviceManager_Dcts_4100 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4200
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4200 start------------------------");
            try {
                dmInstance.on('deviceStateChange', (data) => {
                    console.log("deviceStateChange on:" + JSON.stringify(data));
                    expect(data.action === deviceManager.DeviceStateChange.UNKNOWN).assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceStateChange SUB_DH_DeviceManager_Dcts_4200 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4300 start------------------------");
            try {
                dmInstance.on('deviceStateChange', (data) => {
                    console.log("deviceStateChange on:" + JSON.stringify(data));
                    expect(data.action !== deviceManager.DeviceStateChange.UNAVAILABLE).assertFail();
                    done();
                });
            } catch (err) {
                console.log("deviceStateChange SUB_DH_DeviceManager_Dcts_4300 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4400
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4400 start------------------------");
            try {
                let dmInstanceData = deviceManager.createDeviceManager("");
                console.info("createDeviceManager value is null errCode:" + JSON.stringify(dmInstanceData));
                expect().assertFail();
                done();
            } catch (err) {
                console.error("createDeviceManager 4400 errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4500
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4500 start------------------------");
            try {
                dmInstance.stopDiscovering();
                console.log("stopDiscovering success");
                expect().assertFail();
                done();
            } catch (err) {
                console.info("stopDiscovering errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4600
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4600 start------------------------");
            try {
                let dmInstanceDevice = null;
                deviceManager.releaseDeviceManager(dmInstanceDevice);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_4600 release device manager errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4700
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4700 start------------------------");
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.error("createDeviceManager SUB_DH_DeviceManager_Dcts_4700 errCode:" + err.code + ",errMessage:" + err.message);
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_4700 success");
                let networkIdName = '';
                let deviceName = dmInstance.getDeviceName(networkIdName);
                if (deviceName === null) {
                    console.log('getDeviceName fail; device name BusinessError: ' + JSON.stringify(deviceName));
                }
                console.log('device name: ' + JSON.stringify(deviceName));
                expect(deviceName !== null).assertTrue();
                done();
            } catch (err) {
                console.log('getDeviceName errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4800 start------------------------");
            try {
                let networkIdType = null;
                let deviceType = dmInstance.getDeviceType(networkIdType);
                if (deviceType === null) {
                    console.log('getDeviceType fail; device name BusinessError: ' + JSON.stringify(deviceType));
                }
                console.log('SUB_DH_DeviceManager_Dcts_4800 device type: ' + JSON.stringify(deviceName));
                expect(deviceName !== null).assertTrue();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_4800 getDeviceType errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_4900
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_4900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4900 start------------------------");
            let discoverParam = null;
            let filterOptions = {
                'availableStatus': 0
            };
            try {
                dmInstance.startDiscovering(discoverParam, filterOptions);
                console.log('SUB_DH_DeviceManager_Dcts_4900 startDiscovering success');
                expect().assertFail();
                done();
            } catch (err) {
                console.log('startDiscovering errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_4900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5000
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5000 start------------------------");
            let deviceIdBind = null;
            let bindParam = {};
            try {
                dmInstance.bindTarget(deviceIdBind, bindParam, (err, data) => {
                    if (err) {
                        console.info('bindTarget fail errCode:' + err.code + ',errMessage:' + err.message);
                        return;
                    }
                    console.info('SUB_DH_DeviceManager_Dcts_5000 bindTarget result:' + JSON.stringify(data));
                    expect(data !== null).assertTrue();
                    done();
                });
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5000 bindTarget errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5100 start------------------------");
            try {
                let unDeviceId = '';
                dmInstance.unbindTarget(unDeviceId);
                console.info('SUB_DH_DeviceManager_Dcts_5100 unbindTarget success');
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5100 unbindTarget errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5200
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5200 start------------------------");
            try {
                dmInstance.on(null, (data) => {
                    console.info('SUB_DH_DeviceManager_Dcts_5200 on:' + JSON.stringify(data));
                    expect(data == null).assertTrue();
                    done();
                });
            } catch (err) {
                console.log('on SUB_DH_DeviceManager_Dcts_5200 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5300 start------------------------");
            try {
                dmInstance.off(null, (data) => {
                    console.info('SUB_DH_DeviceManager_Dcts_5200 off:' + JSON.stringify(data));
                    expect(data == null).assertTrue();
                    done();
                });
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5300 off errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5400
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5400 start------------------------");
            try {
                dmInstance.on('deviceStateChange', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5400 on deviceStateChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5500
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5500 start------------------------");
            try {
                dmInstance.off('deviceStateChange', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5500 off deviceStateChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5600
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5600 start------------------------");
            try {
                dmInstance.on('discoverSuccess', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5400 on discoverSuccess errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5700
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5700 start------------------------");
            try {
                dmInstance.off('discoverSuccess', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5500 off discoverSuccess errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5700 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5800 start------------------------");
            try {
                dmInstance.on('deviceNameChange', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5800 on deviceNameChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_5900
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_5900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5900 start------------------------");
            try {
                dmInstance.off('deviceNameChange', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_5900 off deviceNameChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_5900 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6000
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6000 start------------------------");
            try {
                dmInstance.on('discoverFailure', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_6000 on discoverFailure errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6000 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6100
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6100 start------------------------");
            try {
                dmInstance.off('discoverFailure', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_6100 off discoverFailure errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6100 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6200
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6200 start------------------------");
            try {
                dmInstance.on('serviceDie', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_6200 on serviceDie errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6200 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6300 start------------------------");
            try {
                dmInstance.off('serviceDie', null);
                expect().assertFail();
                done();
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_6300 off serviceDie errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6400
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6400 start------------------------");
            try {
                let discoverParam = {
                    'discoverTargetType': 1
                };
                dmInstance.startDiscovering(discoverParam);
                console.info('startDiscovering_1 success');
                try {
                    dmInstance.startDiscovering(discoverParam);
                    console.info('startDiscovering_2 success');
                    expect().assertFail();
                    done();
                } catch (err) {
                    console.log('SUB_DH_DeviceManager_Dcts_6400 startDiscovering_2 errCode:' + err.code + ',errMessage:' + err.message);
                    expect(err.code == 11600104).assertTrue();
                    done();
                }
            } catch (err) {
                console.log('SUB_DH_DeviceManager_Dcts_6400 startDiscovering_1 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code).assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6400 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6500
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this    instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6500 start------------------------");
            dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
            if (!dmInstance) {
                console.error("SUB_DH_DeviceManager_Dcts_6500 createDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
            }
            console.info("SUB_DH_DeviceManager_Dcts_6500 createDeviceManager success");
            deviceId = dmInstance.getLocalDeviceId();
            if (!deviceId) {
                console.log("Dcts_6500 getLocalDeviceId three errCode:" + err.code + ",errMessage:" + err.message);
            }
            console.info("SUB_DH_DeviceManager_Dcts_6500 getLocalDeviceId success, deviceId:" + JSON.stringify(deviceId));
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget(deviceId, bindParam, (err, data) => {
                    if (err) {
                        console.info('bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info('bindTarget 01 result:' + JSON.stringify(data));
                    expect(data == null).assertTrue();
                    try {
                        dmInstance.bindTarget(deviceId, bindParam, (err, data) => {
                            if (err) {
                                console.info('bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                            }
                            console.info('bindTarget 02 result:' + JSON.stringify(data));
                            expect(data == null).assertTrue();
                            done();
                        });
                    } catch (err) {
                        console.info('SUB_DH_DeviceManager_Dcts_6400 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                        expect(err.code == 11600103).assertTrue();
                        done();
                    }
                });
            } catch (err) {
                console.info('SUB_DH_DeviceManager_Dcts_6400 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 11600101).assertTrue();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6500 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_1300
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_1300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1300 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1300 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_1300 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_1300 releaseDeviceManager success");
                    deviceInfoList = dmInstance.getAvailableDeviceListSync();
                    if (!deviceInfoList) {
                        console.log("getAvailableDeviceListSync three errCode:" + err.code + ",errMessage:" + err.message);
                    }
                    expect(deviceInfoList.length).assertEqual(0);
                    done();
                } catch (err) {
                    console.log("getAvailableDeviceListSync SUB_DH_DeviceManager_Dcts_1300 errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_1300 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_1300 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6600
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6600 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_6600 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_6600 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_6600 releaseDeviceManager success");
                    dmInstance.getAvailableDeviceList((err, data) => {
                        console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                        expect(data.length).assertEqual(0);
                        done();
                    });
                } catch (err) {
                    console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_6600 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6600 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6800
         * @tc.name    To manage devices, you must first call this method to obtain a {@code DeviceManager} instance and then
         * use this instance to call other device management methods.
         * @tc.desc    Function test
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_6800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6800 start------------------------");
            let deviceInfoList = null;
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                if (!dmInstance) {
                    console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_6800 fail");
                }
                console.info("createDeviceManager SUB_DH_DeviceManager_Dcts_6800 success");

                try {
                    deviceManager.releaseDeviceManager(dmInstance);
                    console.info("SUB_DH_DeviceManager_Dcts_6800 releaseDeviceManager success");
                    dmInstance.getAvailableDeviceList().then((data) => {
                        console.log('getAvailableDeviceList info: ' + JSON.stringify(data));
                        expect(data.length).assertEqual(0);
                        done();
                    });
                } catch (err) {
                    console.error("getAvailableDeviceList errCode:" + err.code + ",errMessage:" + err.message);
                    expect(err.code == 11600101).assertTrue();
                    done();
                }
            } catch (err) {
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_6800 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
                done();
            }
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6800 end------------------------");
        })

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_6900
         * @tc.name    SUB_DH_DeviceManager_Dcts_6900
         * @tc.desc    Test DeviceStateChange enumeration
         * @tc.size    MEDIUM
         * @tc.type:   Function
         * @tc.level   Level 0
         * @tc.require
         */
        it("SUB_DH_DeviceManager_Dcts_6900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info("-----------------SUB_DH_DeviceManager_Dcts_6900 start------------------------");
            try {
                dmInstance = deviceManager.createDeviceManager("com.ohos.distributedscreenjstest");
                console.log("createDeviceManager SUB_DH_DeviceManager_Dcts_6900 get dmInstance is successfully");
                expect(dmInstance !== null && dmInstance !== undefined).assertTrue();
                expect(deviceManager.DeviceStateChange.UNKNOWN == 0).assertTrue();
                expect(deviceManager.DeviceStateChange.AVAILABLE == 1).assertTrue();
                expect(deviceManager.DeviceStateChange.UNAVAILABLE == 2).assertTrue();
                console.log('createDeviceManger SUB_DH_DeviceManager_Dcts_6900 successfully');
            } catch (err) {
                console.error("createDeviceManger SUB_DH_DeviceManager_Dcts_6900 four errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
            }
            console.log("-----------------SUB_DH_DeviceManager_Dcts_6900 END------------------------");
            done();
        });

        /**
     * @tc.number SUB_DH_DeviceManager_Dcts_7000
     * @tc.name SUB_DH_DeviceManager_Dcts_7000
     * @tc.desc Test getAvailableDeviceListSync interfaces
     * @tc.size MEDIUM
     * @tc.type Function
     * @tc.level Level 2
     * @tc.require
     */
        it('SUB_DH_DeviceManager_Dcts_7000', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + '-----------------SUB_DH_DeviceManager_Dcts_7000 START------------------------');
            try {
                let DeviceBasicInfo = dmInstance.getAvailableDeviceListSync();
                console.info(TAG + 'get deviceInfoList is' + JSON.stringify(DeviceBasicInfo));
                for (let i = 0; i < DeviceBasicInfo.length; i++) {
                    console.info(TAG + 'get deviceInfoList deviceId' + JSON.stringify(DeviceBasicInfo[i].deviceId));
                    console.info(TAG + 'get deviceInfoList deviceName' + JSON.stringify(DeviceBasicInfo[i].deviceName));
                    console.info(TAG + 'get deviceInfoList deviceType' + JSON.stringify(DeviceBasicInfo[i].deviceType));
                    console.info(TAG + 'get deviceInfoList networkId' + JSON.stringify(DeviceBasicInfo[i].networkId));
                    expect(DeviceBasicInfo[i].deviceId != null).assertTrue();
                    expect(DeviceBasicInfo[i].deviceName != null).assertTrue();
                    expect(DeviceBasicInfo[i].deviceType != null).assertTrue();
                    expect(DeviceBasicInfo[i].networkId != null).assertTrue();
                }
                console.info(TAG + 'get deviceInfoList successfully');
                expect(DeviceBasicInfo.length > 0).assertTrue()
            } catch (err) {
                console.info(TAG + "get deviceInfoList failed errCode:" + err.code + ",errMessage:" + err.message);
                expect().assertFail();
            }
            console.info(TAG + '-----------------SUB_DH_DeviceManager_Dcts_7000 END--------------------------');
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7100
         * @tc.name    SUB_DH_DeviceManager_Dcts_7100
         * @tc.desc    The parameter of the createDeviceManager interface is null.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7100 start------------------------");
            try {
                let dmInstanceData = deviceManager.createDeviceManager(null);
                console.info(TAG + "createDeviceManager value is null errCode:" + JSON.stringify(dmInstanceData));
                expect().assertFail();
            } catch (err) {
                console.error(TAG + "createDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7100 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7200
         * @tc.name    SUB_DH_DeviceManager_Dcts_7200
         * @tc.desc    The parameter of the createDeviceManager interface is undefined.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7200 start------------------------");
            try {
                let dmInstanceData = deviceManager.createDeviceManager(undefined);
                console.info(TAG + "createDeviceManager value is null errCode:" + JSON.stringify(dmInstanceData));
                expect().assertFail();
            } catch (err) {
                console.error(TAG + "createDeviceManager errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7200 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7300
         * @tc.name    SUB_DH_DeviceManager_Dcts_7300
         * @tc.desc    The parameter of the releaseDeviceManager interface is empty.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7300 start------------------------");
            try {
                deviceManager.releaseDeviceManager("");
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'release device manager errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7300 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7400
         * @tc.name    SUB_DH_DeviceManager_Dcts_7400
         * @tc.desc    The parameter of the releaseDeviceManager interface is undefined.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7400 start------------------------");
            try {
                deviceManager.releaseDeviceManager(undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'release device manager errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7400 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7500
         * @tc.name    SUB_DH_DeviceManager_Dcts_7500
         * @tc.desc    Get the device name by network id parameter is empty.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7500 start------------------------");
            try {
                let deviceName = dmInstance.getDeviceName("");
                console.log(TAG + "get Device Name:" + deviceName);
                expect().assertFail();
            } catch (err) {
                console.info(TAG + "getDeviceName errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7500 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7600
         * @tc.name    SUB_DH_DeviceManager_Dcts_7600
         * @tc.desc    Get the device name by network id parameter is null.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7600 start------------------------");
            try {
                let deviceName = dmInstance.getDeviceName(null);
                console.log(TAG + "get Device Name:" + deviceName);
                expect().assertFail();
            } catch (err) {
                console.info(TAG + "getDeviceName errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7600 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7700
         * @tc.name    SUB_DH_DeviceManager_Dcts_7700
         * @tc.desc    Get the device name by network id parameter is undefined.
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7700 start------------------------");
            try {
                let deviceName = dmInstance.getDeviceName(undefined);
                console.log(TAG + "get Device Name:" + deviceName);
                expect().assertFail();
            } catch (err) {
                console.info(TAG + "getDeviceName errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7700 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7800
         * @tc.name    SUB_DH_DeviceManager_Dcts_7800
         * @tc.desc    Retrieve the device type by specifying its network identifier,parameter is empty
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7800 start------------------------");
            try {
                let deviceType = dmInstance.getDeviceType("");
                console.log(TAG + 'SUB_DH_DeviceManager_Dcts_7800 device type: ' + JSON.stringify(deviceType));
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'SUB_DH_DeviceManager_Dcts_7800 getDeviceType errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7800 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_7900
         * @tc.name    SUB_DH_DeviceManager_Dcts_7900
         * @tc.desc    Retrieve the device type by specifying its network identifier,parameter is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level 2
         */
        it("SUB_DH_DeviceManager_Dcts_7900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7900 start------------------------");
            try {
                let deviceType = dmInstance.getDeviceType(undefined);
                console.log(TAG + 'device type: ' + JSON.stringify(deviceType));
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'getDeviceType errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_7900 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8000
         * @tc.name    SUB_DH_DeviceManager_Dcts_8000
         * @tc.desc    Test startDiscovering interface parameter is empty;
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8000 start------------------------");
            try {
                dmInstance.startDiscovering("", "");
                console.log(TAG + 'startDiscovering success');
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'startDiscovering errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8000 end------------------------");
            done();
        });

        /*
        * @tc.number  SUB_DH_DeviceManager_Dcts_8100
        * @tc.name    SUB_DH_DeviceManager_Dcts_8100
        * @tc.desc    Test startDiscovering interface parameter is null;
        * @tc.size    MediumTest
        * @tc.type:   Function
        * @tc.level   Level1
        */
        it("SUB_DH_DeviceManager_Dcts_8100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8100 start------------------------");
            try {
                dmInstance.startDiscovering(null, null);
                console.log(TAG + 'startDiscovering success');
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'startDiscovering errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8100 end------------------------");
            done();
        });

        /*
       * @tc.number  SUB_DH_DeviceManager_Dcts_8200
       * @tc.name    SUB_DH_DeviceManager_Dcts_8200
       * @tc.desc    Test startDiscovering interface parameter is undefined;
       * @tc.size    MediumTest
       * @tc.type:   Function
       * @tc.level   Level1
       */
        it("SUB_DH_DeviceManager_Dcts_8200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8200 start------------------------");
            try {
                dmInstance.startDiscovering(undefined, undefined);
                console.log(TAG + 'startDiscovering success');
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'startDiscovering errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8200 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8300
         * @tc.name    SUB_DH_DeviceManager_Dcts_8300
         * @tc.desc    Test that the first parameter of bindTarget is empty
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8300 start------------------------");
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget("", bindParam, (err, data) => {
                    if (err) {
                        console.info(TAG + 'bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info(TAG + 'bindTarget 02 result:' + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8300 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8300 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8400
         * @tc.name    SUB_DH_DeviceManager_Dcts_8400
         * @tc.desc    Test that the first parameter of bindTarget is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8400 start------------------------");
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget(null, bindParam, (err, data) => {
                    if (err) {
                        console.info(TAG + 'bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info(TAG + 'bindTarget 02 result:' + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8400 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8400 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8500
         * @tc.name    SUB_DH_DeviceManager_Dcts_8500
         * @tc.desc    Test that the first parameter of bindTarget is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8500 start------------------------");
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget(undefined, bindParam, (err, data) => {
                    if (err) {
                        console.info(TAG + 'bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info(TAG + 'bindTarget 02 result:' + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8500 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8500 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8600
         * @tc.name    SUB_DH_DeviceManager_Dcts_8600
         * @tc.desc    Test that the second parameter of bindTarget is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8600 start------------------------");
            deviceId = dmInstance.getLocalDeviceId();
            try {
                dmInstance.bindTarget(deviceId, null, (err, data) => {
                    if (err) {
                        console.info(TAG + 'bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info(TAG + 'bindTarget 02 result:' + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8600 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8600 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8700
         * @tc.name    SUB_DH_DeviceManager_Dcts_8700
         * @tc.desc    Test that the second parameter of bindTarget is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8700 start------------------------");
            deviceId = dmInstance.getLocalDeviceId();
            try {
                dmInstance.bindTarget(deviceId, undefined, (err, data) => {
                    if (err) {
                        console.info(TAG + 'bindTarget 02 errCode:' + err.code + ',errMessage:' + err.message);
                    }
                    console.info(TAG + 'bindTarget 02 result:' + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8700 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8700 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8800
         * @tc.name    SUB_DH_DeviceManager_Dcts_8800
         * @tc.desc    Test that the third parameter of bindTarget is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8800 start------------------------");
            deviceId = dmInstance.getLocalDeviceId();
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget(deviceId, bindParam, null);
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8800 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8800 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_8900
         * @tc.name    SUB_DH_DeviceManager_Dcts_8900
         * @tc.desc    Test that the third parameter of bindTarget is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_8900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8900 start------------------------");
            deviceId = dmInstance.getLocalDeviceId();
            let bindParam = {
                "bindType": 1,
                "targetPkgName": "com.ohos.distributedDeviceManager",
                "appName": "SubDctsdisDeviceJsTest",
                "appOperation": "想要连接本机。",
                "customDescription": "device manager"
            };
            try {
                dmInstance.bindTarget(deviceId, bindParam, undefined);
            } catch (err) {
                console.info(TAG + 'SUB_DH_DeviceManager_Dcts_8900 bindTarget 01 errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_8900 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9000
         * @tc.name    SUB_DH_DeviceManager_Dcts_9000
         * @tc.desc    Test unbindTarget parameter is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9000 start------------------------");
            try {
                dmInstance.unbindTarget(null);
                console.info(TAG + 'unbindTarget success');
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'unbindTarget errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9000 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9100
         * @tc.name    SUB_DH_DeviceManager_Dcts_9100
         * @tc.desc    Test unbindTarget parameter is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9100 start------------------------");
            try {
                dmInstance.unbindTarget(undefined);
                console.info(TAG + 'unbindTarget success');
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'unbindTarget errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9100 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9200
         * @tc.name    SUB_DH_DeviceManager_Dcts_9200
         * @tc.desc    Test that the second parameter of on_deviceStateChange is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9200 start------------------------");
            try {
                dmInstance.on('deviceStateChange', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on deviceStateChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9200 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9300
         * @tc.name    SUB_DH_DeviceManager_Dcts_9300
         * @tc.desc    Test that the second parameter of off_deviceStateChange is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9300 start------------------------");
            try {
                dmInstance.off('deviceStateChange', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on deviceStateChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9300 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9400
         * @tc.name    SUB_DH_DeviceManager_Dcts_9400
         * @tc.desc    Test that the first parameter of on is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9400 start------------------------");
            try {
                dmInstance.on(null, (data) => {
                    console.log(TAG + "on_null:" + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.log(TAG + "on_null errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9400 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9500
         * @tc.name    SUB_DH_DeviceManager_Dcts_9500
         * @tc.desc    Test that the first parameter of on is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9500 start------------------------");
            try {
                dmInstance.on(undefined, (data) => {
                    console.log(TAG + "on_null:" + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.log(TAG + "on_null errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9500 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9600
         * @tc.name    SUB_DH_DeviceManager_Dcts_9600
         * @tc.desc    Test that the first parameter of on is null
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9600", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9600 start------------------------");
            try {
                dmInstance.off(null, (data) => {
                    console.log(TAG + "on_null:" + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.log(TAG + "on_null errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9600 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9700
         * @tc.name    SUB_DH_DeviceManager_Dcts_9700
         * @tc.desc    Test that the first parameter of on is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9700", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9700 start------------------------");
            try {
                dmInstance.off(undefined, (data) => {
                    console.log(TAG + "on_null:" + JSON.stringify(data));
                    expect().assertFail();
                });
            } catch (err) {
                console.log(TAG + "on_null errCode:" + err.code + ",errMessage:" + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9700 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9800
         * @tc.name    SUB_DH_DeviceManager_Dcts_9800
         * @tc.desc    Test that the second parameter of on_discoverSuccess is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9800", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9800 start------------------------");
            try {
                dmInstance.on('discoverSuccess', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on discoverSuccess errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9800 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_9900
         * @tc.name    SUB_DH_DeviceManager_Dcts_9900
         * @tc.desc    Test that the second parameter of off_discoverSuccess is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_9900", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9900 start------------------------");
            try {
                dmInstance.off('discoverSuccess', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'off discoverSuccess errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_9900 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10000
         * @tc.name    SUB_DH_DeviceManager_Dcts_10000
         * @tc.desc    Test that the second parameter of on_deviceNameChange is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10000", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10000 start------------------------");
            try {
                dmInstance.on('deviceNameChange', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on deviceNameChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10000 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10100
         * @tc.name    SUB_DH_DeviceManager_Dcts_10100
         * @tc.desc    Test that the second parameter of off_deviceNameChange is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10100", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10100 start------------------------");
            try {
                dmInstance.off('deviceNameChange', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on deviceNameChange errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10100 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10200
         * @tc.name    SUB_DH_DeviceManager_Dcts_10200
         * @tc.desc    Test that the second parameter of on_discoverFailure is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10200", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10200 start------------------------");
            try {
                dmInstance.on('discoverFailure', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on discoverFailure errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10200 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10300
         * @tc.name    SUB_DH_DeviceManager_Dcts_10300
         * @tc.desc    Test that the second parameter of off_discoverFailure is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10300", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10300 start------------------------");
            try {
                dmInstance.off('discoverFailure', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'off discoverFailure errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10300 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10400
         * @tc.name    SUB_DH_DeviceManager_Dcts_10400
         * @tc.desc    Test that the second parameter of on_serviceDie is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10400", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10400 start------------------------");
            try {
                dmInstance.on('serviceDie', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'on serviceDie errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10400 end------------------------");
            done();
        });

        /*
         * @tc.number  SUB_DH_DeviceManager_Dcts_10500
         * @tc.name    SUB_DH_DeviceManager_Dcts_10500
         * @tc.desc    Test that the second parameter of off_serviceDie is undefined
         * @tc.size    MediumTest
         * @tc.type:   Function
         * @tc.level   Level1
         */
        it("SUB_DH_DeviceManager_Dcts_10500", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function (done) {
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10500 start------------------------");
            try {
                dmInstance.off('serviceDie', undefined);
                expect().assertFail();
            } catch (err) {
                console.log(TAG + 'off serviceDie errCode:' + err.code + ',errMessage:' + err.message);
                expect(err.code == 401).assertTrue();
            }
            console.info(TAG + "-----------------SUB_DH_DeviceManager_Dcts_10500 end------------------------");
            done();
        });
    })
}