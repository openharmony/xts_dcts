...
//static SessionAttribute* g_sessionAttr4Ctrl = NULL;
static SessionAttribute* g_sessionAttr4Data = NULL;
//static ISessionListener* g_sessionlistener4Ctrl = NULL;
static ISessionListener* g_sessionlistener4Data = NULL;

//static char g_dataMgrReply[1024] = {0};
static char g_networkId[NETWORK_ID_BUF_LEN] = {0};
static INodeStateCb g_defNodeStateCallback;
static ConnectionAddr g_ethAddr = {.type = CONNECTION_ADDR_WLAN,};
...
static void OnDefNodeOnline(NodeBasicInfo* info)
{
	...
	if(info == NULL)
	{ 
        LOG("OnDefNodeOnline info is null");
		return;
	}
...
static void OnDefNodeBasicInfoChanged(NodeBasicInfoType type, NodeBasicInfo* info)
{
    if(info == NULL)
	{ 
        LOG("OnDefNodeBasicInfoChanged info is null");
		return;
	}
...
static void DataMessageReceived(int sessionId,const void* data, unsigned int dataLen)
{
	...
	if(dataLen <= maxLen)
	{
		...
		memset(buf, 0, 1024);
		strncpy((char*)buf,(char*)data, dataLen);
		//strcpy((char*)buf,(char*)data);

		if(*code != -1)
		{
		    pthread_t thread;
			int ret = pthread_create(&thread, NULL, DataOperateTask, buf);
...
int CreateSsAndOpenSession4Data(void)
{
	...
	pthread_create(&sendThread, NULL, SendMsgTask, NULL);
...
int main(int args, char* argv[])
{
	...
	/*int ret = DiscoverAndJoinNetwork();
	if(ret != SOFTBUS_OK)
	{
	    LOG("discovert failed ret=%d",ret);
		StopDiscovery(DEF_PKG_NAME, g_subscribeId);
	}
